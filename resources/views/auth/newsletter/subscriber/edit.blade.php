        ...
        <div class="col-md-6 col-md-offset-3">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <form action="{{ route('admin.subscriber.edit.post') }}" method="post" role="form" data-toggle="validator" id="subscriber-form">
                    <input type="hidden" name="id" value="{{ $subscriber->id }}">
                    {{ csrf_field() }}
                    {{ method_field('post') }}
                        <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                            <label for="name" class="control-label">@lang('newsletter.subscribers.form.name')</label>
                            <input type="text" name="name" value="{{ old('name', $subscriber->name) }}" class="form-control" required>
                            <span class="help-block">{{ $errors->first('name') }}</span>
                        </div>
                        <div class="form-group {{ $errors->has('email') ? 'has-error' : '' }}">
                            <label for="email" class="control-label">@lang('newsletter.subscribers.form.email')</label>
                            <input type="email" name="email" value="{{ old('email', $subscriber->email) }}" class="form-control" required>
